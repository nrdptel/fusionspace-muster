 ...
 *
 *  The {" "} after a closing tag is load-bearing, not formatting noise: a run of JSX text that
 *  spans two lines AND carries an entity (&apos;, &nbsp;, &ldquo;) loses its leading space at
 *  compile time, gluing "crossload" to "caution". components/jsx-whitespace.test.ts guards it. */
      ...
      <div className="mt-4">
        <Disclosure summary="How a reload fits a case — direct vs. spacer">
          ...
          <p>
            ...
            <strong>38&nbsp;mm</strong> and Cesaroni <strong>Pro29/38/54</strong>{" "}
            — and elsewhere (AeroTech 29/54&nbsp;mm, Cesaroni Pro24/75/98 and the 6GXL cases) it
            flags that spacers exist and points you to the instructions rather than asserting a step
            it can&apos;t source. <strong>Loki Research</strong> publishes no spacer system at all,
            so its reloads are direct-fit only — each one matched to its own case.
          </p>
          <p>
        ...
        <Disclosure summary="Cross-brand crossloads (75 & 98 mm)">
          <p>
            ...
            hardware. Muster shows these as a distinct <strong>crossload</strong>{" "}
            caution — never a green &ldquo;direct fit&rdquo; — with each maker&apos;s own
            conditions attached.
          </p>
          <p>
            ...
            <strong>AeroTech advises against</strong>{" "}
            — the RMS case wants a forward seal disc the Cesaroni reload doesn&apos;t include, and
            AeroTech won&apos;t warranty it. Either way, a cross-brand motor has to be certified in
            the hardware it&apos;s flown in.
          </p>
          <p>
            Nothing at <strong>29/38/54&nbsp;mm</strong>{" "}
            is a crossload: no manufacturer or certification body sanctions it, and the closures and
            reload formats differ, so Muster doesn&apos;t show it — a wrong cross-brand pairing is
            exactly the kind of edge that ends in a CATO.
          </p>
        </Disclosure>

        <Disclosure summary="What you own vs. what's in the reload">
          <p>
            The reusable metal <strong>hardware</strong>{" "}
            you buy once, shared across every case length in a diameter. For AeroTech RMS
            that&apos;s the case, a forward closure, an aft closure, and — on the longer cases — a
            forward seal disc. Cesaroni Pro reuses less: on{" "}
            ...
            <strong>Loki Research</strong>{" "}
            reuses the most — the case, the forward bulkhead, and the graphite nozzle all carry
            over, so its reload is only the liner, grains, o-rings, and a delay or smoke element.
            Loki&apos;s nozzle is sold by throat number and must match the reload; some longer
            54&nbsp;mm reloads need its extended bulkhead.
          </p>
          <p>
        ...
        <Disclosure summary="Certification, and what it doesn't mean">
          ...
          <p>
            <strong>Out of production is not the same as decertified.</strong>{" "}
            An out-of-production reload stays flyable until stocks run out; Muster marks it so you
            know it may be hard to find, not that it&apos;s disallowed. Muster never marks a motor
            decertified on its own — that&apos;s a call for the cert organizations, and their
